...
We organize results into five categories: end-to-end timing, energy consumption, per-phase analysis, system utilization, and instrumentation overhead. All figures referenced in this section are collected in Figure~\ref{fig:all}.
...
\begin{figure*}[t]
    \centering
    % --- Row 1: per-phase timing ---
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/phase_time_bars_disk_w0_b32.png}
        \caption{Phase timing (disk, w=0, B=32).}
        \label{fig:phase_bars}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/phase_time_bars_mila_w0_b32.png}
        \caption{Phase timing (Milabench, w=0, B=32).}
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        ...
        \caption{Per-phase resource violins (disk, w=0, B=32).}
        \label{fig:phase_violins}
    \end{subfigure}

    \vspace{0.15cm}

    % --- Row 2: GPU utilization ---
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/gpu_util_disk_w0.png}
        \caption{GPU util.\ across batch sizes (disk, w=0).}
        \label{fig:gpu_util}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\textwidth}
        ...
        \caption{GPU and CPU util.\ (Milabench, w=0, B=32).}
        \label{fig:gpu_util_mila}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/gpu_util_disk_w4.png}
        \caption{GPU util.\ across batch sizes (disk, w=4).}
        \label{fig:gpu_util_w4}
    \end{subfigure}

    \vspace{0.15cm}

    % --- Row 3: resource timeline + overhead ---
    \begin{subfigure}[t]{0.32\textwidth}
        \centering
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        ...
        \caption{Instrumentation overhead (workers=0).}
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        ...
        \caption{Instrumentation overhead (workers=4).}
        \label{fig:overhead}
    \end{subfigure}

    \caption{Experimental results. \textbf{Row~1}: Per-phase timing shows backward pass dominating at ${\sim}68\%$; violin plots confirm GPU util.\ drops during forward and spikes during optimizer. \textbf{Row~2}: GPU utilization exceeds 95\% at steady state for both data backends; workers=4 introduces periodic CPU spikes from background loaders. \textbf{Row~3}: Full resource timeline shows stable utilization after warm-up; instrumentation overhead is 30--36\% with workers=0 (dominated by CUDA sync) but only 1--7\% with workers=4.}
    \label{fig:all}
\end{figure*}
